...
def runMain(argv):
  ...
  factFile = 'argmining.cfacts'
  trainFile = 'inferred_type_learn.exam'
  testFile = 'inferred_type_eval.exam'

  # construct a Compiler object
  tlog = simple.Compiler(db=factFile,prog='argmining.tlog')
  ...
  tlog.mark_db_predicate_trainable('reln/2')
  #tlog.mark_db_predicate_trainable('has_stance/2')
  tlog.mark_db_predicate_trainable('has_type/2')

  # compile the rules, plus a query mode, into the inference function,
  # which we will use for testing
  mode = 'inferred_type/io'
