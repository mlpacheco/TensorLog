...
def runMain(argv):
  ...
  # runMain(["--epochs","20","--issue","evolution","--fold", "f0"])
  opts = simple.Options()
  opts.regularizer_scale = 0.1
  opts.link_scale = 0.9
  opts.epochs = 20 # 0 for no learning
  opts.max_depth = 100
  opts.issue = "evolution"
  opts.fold = "f0"
  ...
  factFile = '{0}/{1}/all.cfacts'.format(opts.issue, opts.fold)
  trainFile = '{0}/{1}/inferred_stance_learn.exam'.format(opts.issue, opts.fold)
  testFile = '{0}/{1}/inferred_stance_eval.exam'.format(opts.issue, opts.fold)
  mode = 'inferred_stance/io'

  testFile_agree = '{0}/{1}/inferred_agree_eval.exam'.format(opts.issue, opts.fold)

  print(factFile, trainFile, testFile)

  # construct a Compiler object
  tlog = simple.Compiler(db=factFile,prog='4forums.tlog')
  ...
  tlog.mark_db_predicate_trainable('agree/2')
  tlog.mark_db_predicate_trainable('stance/2')
  ...
  for i in range(opts.epochs):
    ...
    print('epoch',i+1,'train loss, accuracy and macro f1',session.run([unregularized_loss,accuracy], feed_dict=train_fd),macro_f1,macro_f1_test)
  ...
  #schema_fp = open("schema_fp_{0}_{1}".format(opts.issue, opts.fold), "w")
  #data_stream = open("data_stream_{0}_{1}".format(opts.issue, opts.fold), "wb")
